...
Прошивка ESP32 (опкод 0x0E ``BENCH_RUN``) возвращает ``n_samples`` (big-endian
u16) и затем поток сэмплов ``{t_us:u32, i_raw:i16, p_raw:u16}`` по 8 байт.
Сэмплы пишутся сырым memcpy структуры на little-endian ESP32, поэтому здесь
они распаковываются как little-endian.

Модуль парсит байтовый поток в numpy-массивы, конвертирует raw в SI по
калибровке INA219 (Current_LSB=100 мкА, Power_LSB=2 мВт из ``ina219_init()``)
и интегрирует мощность по времени методом трапеций в энергию обновления.
"""
