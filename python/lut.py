...
- байты 0..59  — ``VS[nX-LUTm]``: источник напряжения для каждой фазы n=0..11,
                 sub-frame X=A,B,C,D, sub-LUT m=0..4 (2 бита на значение) → 60 байт.
- байты 60..143 — тайминги, по 7 байт на фазу с чередованием:
                 ``TP_A, TP_B, SR_AB, TP_C, TP_D, SR_CD, RP`` → 12·7 = 84 байта.
- байты 144..149 — ``FR[n]``: frame rate каждой фазы (4 бита, 2 фазы/байт) → 6 байт.
- байты 150..152 — ``XON[nXY]``: gate gating sub-frame pair (1 бит, 8/байт) → 3 байта.

Итого 60 + 84 + 6 + 3 = 153 байта.
...
class Source(IntEnum):
    ...
    VSH2 = 0b11  # второй положительный источник (в модели работы +5 В)
