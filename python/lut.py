...
Жёсткое физическое ограничение **charge balance**:
    Σ_k V[k] · TP[k] = 0  для каждого пикселя.
Если константа не нулевая, на панель накапливается DC-смещение, что приводит
к stuck pixels и преждевременному износу (см. RESEARCH.md → INSIGHT I-06).
"""
...
class Lut:
    ...
    def validate(self, charge_tolerance: float = 1e-9) -> list[str]:
        """Проверить consistency LUT. Возвращает список ошибок (пустой = OK).

        Параметры:
            charge_tolerance: допустимый по модулю charge balance
                (в условных «вольт × frame», см. ``charge_balance``).
        """
        errors: list[str] = []
