...
% ---------- 7b. Постановка и какой это Парето ----------
\begin{frame}{Постановка задачи и какой это Парето-фронт}
  Дискретная задача теоремы~4.1 на калиброванной панели (одно-кадровый переход):
  \vspace{-1mm}
  \begin{block}{}
    \[
      \min_{T_c,\,T_d}\; E(w)\quad\text{при}\quad
      C(w)\ge C^{*},\;\; \tau(w)\le\tau_{\max},\;\; |Q(w)|\le\varepsilon,\;\; T_c\ge T_c^{\min}
    \]
    \vspace{-3mm}
    \centerline{\footnotesize $Q(w)=V(T_d-T_c)$ — чистый заряд (DC-баланс); $T_c^{\min}$ — минимальный клир против гостинга}
  \end{block}
  \vspace{1mm}
  Общий метод трёхкритериален $(E,G,\tau)$, но здесь он сворачивается до
  \alert{2D «энергия~$\leftrightarrow$~контраст»}:
  \begin{itemize}
    \item $\tau$ \textbf{схлопывается в} $E$: обе линейны по числу кадров $N$
      $\Rightarrow$ коллинеарны (одна ось, минимум $E$ = минимум $\tau$)
    \item гостинг $G$ — \textbf{структурно} через $T_c\ge T_c^{\min}$ + отдельная проверка на стенде (не накапливается)
    \item контраст $C \ne$ гостинг $G$: $C$ — качество \emph{нового} кадра; $G$ — остаток \emph{предыдущего}
  \end{itemize}
\end{frame}
