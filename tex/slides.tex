...
% ---------- 7b. Какой это Парето ----------
\begin{frame}{Какой это Парето-фронт: 3 критерия $\to$ 2D}
  Общий метод трёхкритериален $(E, G, \tau)$. На \emph{калиброванной панели} он
  сворачивается до двумерного компромисса \alert{энергия~$\leftrightarrow$~контраст}:
  \vspace{1mm}
  \begin{itemize}
    \item \textbf{$\tau$ схлопывается в $E$}: обе линейны по числу кадров $N$
      $\Rightarrow$ $\tau = \tau_0 + \tfrac{k_\tau}{k_E}(E - E_0)$ — коллинеарны,
      это одна ось (минимум энергии = минимум времени)
    \item \textbf{Гостинг $G$ не свайпится как цель}, а держится структурно:
      ограничение $T_c \ge T_c^{\min}$ (минимальный клир) + отдельная проверка на
      стенде — накопления нет
    \item \textbf{Контраст $C \ne$ гостинг $G$}: $C$ — качество \emph{нового}
      изображения (разделённость ч/б); $G$ — остаток \emph{предыдущего} кадра
  \end{itemize}
  \vspace{1mm}
  \begin{block}{}
    Поэтому активный компромисс, который перебирает оптимизатор, — энергия против
    контраста. Это и есть измеренный Парето-фронт (слайд далее).
  \end{block}
\end{frame}
