...
\begin{frame}{Проблема: обновление e-ink стоит дорого}
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      \begin{itemize}
        \item EPD (электронная бумага): читалки, ценники, IoT-индикаторы~--- сегменты с критичным энергопотреблением
        \item Статичное изображение энергии не требует, \alert{вся цена~--- в моменте обновления}
        \item Обновлением управляет \textbf{waveform}~--- программа импульсов напряжения (LUT контроллера)
        \item Заводские waveform универсальны и консервативны: длинная осцилляция, запас по температуре
      \end{itemize}
      \vspace{2mm}
      \begin{block}{Вопрос работы}
        Сколько энергии и времени можно сэкономить, проектируя waveform \emph{оптимально},~--- и что говорит теория о форме оптимума?
      \end{block}
    \end{column}
...
\begin{frame}{Аппаратный стенд}
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      \centering
      \includegraphics[width=\textwidth]{figures/raw_photos/stand_overview.jpg}\\
...
\begin{frame}{Метрика качества: выбрана эмпирически}
  \begin{columns}[T]
    ...
    \begin{column}{0.43\textwidth}
      \centering
      \includegraphics[width=\textwidth]{figures/raw_photos/roi_overlay.jpg}\\
...
\begin{frame}{Калибровка панельной модели: $R^2 \approx 1$}
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      \vspace{2mm}
      $$E(N) = 1{,}82 + 0{,}088\,N \;\;\text{мДж} \quad (R^2{=}0{,}996)$$
      $$\tau(N) = 0{,}27 + 0{,}020\,N \;\;\text{с} \quad (R^2{=}1{,}000)$$
      \vspace{-2mm}
      \begin{itemize}
        \item Частота кадров из наклона: $f = 50{,}1$~Гц~--- совпала с допущением модели (независимое подтверждение)
        \item Контраст: насыщающаяся модель по фазам $T_c, T_{\mathrm{osc}}, T_d$ ($R^2{=}0{,}978$); осцилляция вчетверо менее эффективна, чем финальный драйв
...
\begin{frame}{Проверка на реалистичном контенте}
  \centering
  \includegraphics[width=0.9\textwidth]{figures/raw_photos/scenarios_panels.png}\\[1mm]
  ...
  \begin{itemize}
    ...
    \item Переключение контента без промежуточной очистки: остаточный след $<1{,}5\,\%$ у обоих~--- экономия не ухудшает ghosting
    \item Циклический тест (8 обновлений подряд): накопления гостинга нет (0{,}099 против 0{,}105 у заводской)
  \end{itemize}
\end{frame}
...
\begin{frame}{Динамический контент: видео и пинг-понг}
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \vspace{1mm}
      \centering
      \includegraphics[width=0.92\textwidth]{figures/raw_photos/pong_frames.png}\\
