...
параметрах она составляет единицы миллисекунд.
...
Гаусса--Остроградского и в силу произвольности $\Omega$ получаем локальную
форму:
...
Подставив~\eqref{eq:flux-final} в~\eqref{eq:continuity} и считая $D_\pm$,
$\mu_{e,\pm}$ постоянными в объёме капсулы (изотропная жидкость), получаем
...
условие~\eqref{eq:bc-neumann} означает отсутствие реакций на электродах и
...
PNP-система сводится \emph{редукцией методом среднего поля} (mean-field
reduction) к ОДУ для центра масс распределения частиц одного пикселя.
...
моделируется на уровне \emph{панели}, в виде функций $E(N)$ и $\tau(N)$ числа
кадров waveform. Вывод, коэффициенты ($R^2\approx1$) и
проверка приведены в гл.~\ref{ch:experiment}, \S\ref{sec:calib}. Предварительные
