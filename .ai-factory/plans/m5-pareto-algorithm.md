...
- [x] **E1** ← D1 — Заполнить `tex/chapters/05_algorithm.tex` (удалить «Заглушку», добавить ~5 стр согласно TODO-структуре). 5 секций:
...
- [x] **F1** ← E1 — Заполнить `tex/chapters/06_simulation.tex` (~5 стр согласно TODO-структуре). 5 секций:
...
- [x] **G1** ← F1 — Review глав 5-6 одобрен (включая deep revision: B0-симулированный, NSGA-II hard-reject + smart init, расширение ε-сетки до 0.7, perf-оптимизация 76с→2.7с, 3D-фигура pareto_vs_nsga2_3d добавлена) + проверки рисунков и таблиц (вёрстка не плывёт, подписи по ГОСТ). Verify: `\ref{sec:eps-constraint}` в § 4.6 теперь резолвится (нет «??» в PDF).

### Phase H: Финализация (1 задача)

- [x] **H1** ← G1 — Final `make build`, проверка `dist/thesis.pdf` (~58 стр суммарно: было 53 после M4 + ~10 за гл. 5 и 6 − небольшая экономия от слияния заглушек), ROADMAP: M5 → [x] + строка `M5. Pareto algorithm + simulator | <дата>` в Completed table. Обновить RESEARCH.md: 1-2 INSIGHT-карточки про численные находки (например, насколько Парето-фронт оптимизатора превзошёл заводской LUT; насколько близок к нижней границе Theorem 4.2).
