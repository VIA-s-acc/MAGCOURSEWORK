...
- [x] **D1 (Task #20)** — извлечь главы Васильева 1-5,8 и Понтрягина 1-3 через `pdftotext`; структурированные конспекты в `docs/notes/vasiliev.md`, `docs/notes/pontryagin.md` (определения + теоремы + страницы). **Примечание:** PDFs оказались image-only без text layer (typical для русских отсканированных книг), полный OCR не делался. Конспекты составлены как структурированные skeleton'ы с TOC по канону + ключевые ссылки на главы курсовой + явные TODO для ручной выписки. OCR опционально через `ocrmypdf --language rus`.
- [x] **D2 (Task #21)** — конспект Paruchuri-Chatterjee 2019 целиком (31 стр., полный текст, содержательный конспект с Theorem 3.1, ремарками и явным сопоставлением с нашей формулировкой) и Моисеева главы 1-3 (skeleton — DJVU image-only, без OCR); `docs/notes/paruchuri.md`, `docs/notes/moiseev.md`. **Уточнение:** Phogat 2017 оказался ошибочной атрибуцией — реальный автор Paruchuri (поправлено в bib и RESEARCH).
- [x] **D3 (Task #22)** — `docs/notes/notation_map.md`: таблица соответствий обозначений (Paruchuri ↔ Понтрягин ↔ Васильев ↔ наша курсовая), плюс локальные обозначения курсовой (z, u, U, E, G, τ), соглашения по индексам и шрифтам.
- [x] **D4 (Task #23)** — `docs/notes/proof_sketches.md`: эскиз доказательства главной теоремы (bang-bang ≤K_eff в charge-balanced классе) на основе D1-D3. **Декларация: эскиз складывается.** Все шаги опираются на Paruchuri 2019 Theorem 3.1 + Pontryagin §3.3 + Васильев гл.5. K_eff = d_state + 2 = 4 для нашего 2D state. Зафиксированы 2 уровня fallback (слабее, ещё слабее) на случай если строгая оценка K_eff не получится. Открытые вопросы — технические уточнения, не препятствия.
