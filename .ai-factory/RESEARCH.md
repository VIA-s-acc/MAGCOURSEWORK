...
16. **Phogat K., Banavar R., Chatterjee D.** (2017). *Discrete time Pontryagin maximum principle for optimal control problems under state-action-frequency constraints.* arXiv:1708.04419.
    - URL: [arxiv.org/abs/1708.04419](https://arxiv.org/abs/1708.04419)
    - VERIFIED: arXiv.
    - Применение: современная формулировка для constraints на состояние И на множество допустимых управлений — наш случай (управление дискретно по уровню напряжения).
...
| `phogat2017_discrete_pmp.pdf` | Phogat K., Banavar R., Chatterjee D. (2017). arXiv:1708.04419. | 31 |
...
|---|---|---|---|---|
| **0. Setup** | W1 (текущая) | — | Прочитан Васильев гл.1-5 (методы оптимизации, основные обозначения), Понтрягин гл.1-3 (PMP канон), Phogat 2017 целиком (дискретный PMP). LaTeX-каркас по ГОСТ. | Расширить прошивку ESP32 — опкод динамической записи LUT через USB (если ещё нет). |
...
LUT'а — 5 sub-LUT × 12 фаз × 4 sub-frame × алфавит источников {VSH1, VSH2, VSL, VCOM, 0}. **Без всякой натяжки** это попадает в класс задач дискретного оптимального управления (Phogat-Banavar-Chatterjee 2017, arXiv:1708.04419). Это даёт нам право использовать дискретный PMP Болтянского для нашей теоремы.
→ **Применение:** § 4.1, § 4.4.
...
- Скачана статья Phogat-Banavar-Chatterjee 2017 (arXiv:1708.04419), `docs/refs/phogat2017_discrete_pmp.pdf`, 31 стр. — основной технический референс для теоремы.
...
- `docs/refs/phogat2017_discrete_pmp.pdf`
