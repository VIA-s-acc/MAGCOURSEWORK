...
### I-11. Yang 2021 — индустриальный consensus про charge balance
Цитата из Yang 2021 (mechanisms review): «the length of pushing the particle in a particular direction should be the same as the negative pulse length ... so that no charge accumulates». Это **прямое индустриальное обоснование** нашего hard-constraint `Σ V·TP = 0`. Не теоретическая гипотеза — наблюдаемая в industry практика, без которой ресурс панели падает.
→ **Применение:** § 4.3 (формальная вставка hard-constraint), § 1.1 (актуальность ресурса как 4-й цели).

### I-12. Lin 2024 — формула P_peak подставляется в наш функционал прямо
`P_peak = ½·C·ΔV²·f·V_source`. ΔV — амплитуда **скачка** между фазами k и k+1. f — частота фазовых переключений. Это даёт `E[k] = P_peak[k]·TP[k]`. У нас в symbolic виде: `α·E = α · ½·C·V_source · Σ_k ΔV_k²·f·TP[k]`. Для минимизации энергии стремимся уменьшить число резких скачков ΔV — что подталкивает к bang-bang с **минимальным** числом фаз (что **точно совпадает с нашей Теоремой 4.1**).
→ **Применение:** § 4.2 (вывод явной формы функционала), § 4.6 (следствие теоремы — связь с Lin 2024 эмпирикой).

### I-13. Wang 2022 + He 2020 — Стоксова формула как первое приближение
Wang даёт `v = (Uq/6πdηR)·(1 − exp(-6πηR·t/m))`, He даёт более общий вид через 2-й закон Ньютона `F = qE − 6πμvr = m·dv/dt`. Это идентичные модели в разных обозначениях. Постоянная времени `τ_Stokes = m/(6πμR)` — единственная физическая константа, которую мы калибруем из данных стенда.
→ **Применение:** § 3.3 (mean-field редукция), § 3.5 (калибровка).

### I-14. Bert 2003 — ограничения чисто электрофоретической модели
Bert показывает что в неоднородном поле существенен diэлектрофорез `F_DEP ∝ ∇|E|²`. В нашем режиме (постоянные импульсы, charge-balanced) DEP вносит **второстепенный вклад** относительно EP, что обосновывает использование Стоксовой модели как первого приближения. Однако нужно **явно проговорить** это допущение в § 3.1, чтобы избежать критики «работа физически неполна».
→ **Применение:** § 3.1 (обоснование mean-field), § 8 (DEP-extension как future work).

### I-15. He 2020 — частный случай нашей теоремы (inflection-point heuristic)
He эмпирически нашли что optimal `T1 = 60 ms` соответствует **точке перегиба** brightness curve. С точки зрения нашей формальной теории — это момент когда `d²z/dt² = 0` в lumped ODE-модели, что соответствует переключению Hamiltonian'а с одного `arg max` на другой в дискретной PMP-постановке. То есть He эмпирически нашли **один** switching point — наша теорема даёт **все** switching points с явной верхней границей `K_eff`.
→ **Применение:** § 4.5 (физическая интерпретация bang-bang structure), § 7.4 (сравнительная валидация с He 2020 как baseline).

### I-16. Kang 2025 — алгоритмическое vs структурное различие
Kang применяет DP, гарантирующее глобальный оптимум в их state-action graph при заданной скаляризации. Это **алгоритмический** результат. Наша теорема даёт **структурный** результат — форму оптимума для **любого** монотонного скалярного функционала, без необходимости полного перебора. На практике это значит:
- DP Kang: для каждого нового набора весов (α, β, γ) — пересчёт всего графа.
- Наш PMP: одна теорема, генерирующая все Парето-точки через ε-constraint без пересчётов.
→ **Применение:** § 5.4 (детальное сравнение complexity и applicability), § 2.4 (gap analysis).
